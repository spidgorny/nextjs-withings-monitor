...
async function getNonce(timestamp: number): Promise<string> {
	...
	try {
		const response = await axios.post(`${API_ENDPOINT}/v2/signature`, new URLSearchParams({ ...params, signature }), {
			headers: {
...
async function createUser(userData: UserData): Promise<string> {
	...
	try {
		...
		const response = await axios.post(`${API_ENDPOINT}/v2/sdk`, new URLSearchParams(requestBody), {
			headers: {
				'Content-Type': 'application/x-www-form-urlencoded',
			},
		});
