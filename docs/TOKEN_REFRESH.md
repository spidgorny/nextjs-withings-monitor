...
## Problem
When access tokens expire (401 error with "invalid_token"), the system needs to automatically refresh them using the refresh token and update the `.env.json` file so subsequent requests work.
