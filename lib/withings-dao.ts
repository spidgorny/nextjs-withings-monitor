...
export class WithingsDAO {
	 ...
	 */
	async fetchAndStore(accessToken: string, userid: string, year: number, month: number): Promise<MeasurementData> {
	 ...
	 */
	async store(userid: string, year: number, month: number, data: MeasurementData): Promise<void> {
	 ...
	 */
	async read(userid: string, year: number, month: number): Promise<MeasurementData | null> {
		try {
			const filePath = this.getFilePath(userid, year, month);
