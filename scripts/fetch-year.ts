...
async function loadConfig(): Promise<EnvConfig> {
    try {
        ...
        return {
            WITHINGS_ACCESS_TOKEN: json.WITHINGS_ACCESS_TOKEN || json.access_token,
            WITHINGS_USER_ID: json.WITHINGS_USER_ID || json.userid,
        }
    } catch (error) {
...
if (isNaN(year) || year < 2000 || year > 2100) {
    console.error('Invalid year. Usage: npx tsx scripts/fetch-year.ts [YEAR]');
    process.exit(1);
}
